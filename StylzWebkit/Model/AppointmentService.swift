...
//    public var employee_service_price : String?//
    ...
    public init(json:JSON) {
      ...
      // service_amount = json["service_amount"].intToSafeString()
        ...
        service_amount = String(format: "%.1f", json["service_amount"].doubleValue)
        
        print("~~ service amount  \(service_amount) = \(json["service_amount"].doubleValue)")
        ...
        if let choice = json["employee_image"].dictionary {
            employee_image = choice["image"]?.string
            }
